...
function Particle(x, y, dummy){
  this.origX = this.x = x;
  this.origY = this.y = y;
  this.color = !dummy ? "white" : 'gray';
...
logo.onload = function(){
  ...
  textCtx.font = textCanvas.height + 'px serif';
  textCtx.textBaseline = 'top';
  textCtx.fillText('this is test text ', 0, 0);

  var textData = textCtx.getImageData(0,0, textCanvas.width,textCanvas.height),
      pixels2 = textData.data;

  for(var y =0; y < textData.height; y+= 10) {

    for(var x = 0; x < textData.width; x+= 10) {

      var alpha = pixels2[];

      if(alpha > 0){
        logoParticles.push(new Particle(x, y, false));
      }else{
        // bgParticles.push(new Particle(x, y, true));
      }
    }
  }
